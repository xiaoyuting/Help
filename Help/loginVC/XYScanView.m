...
- (void)layoutSubviews{
    ...
    //扫描框的宽
    CGFloat scanViewW = KScreenWidth-120;
    
    //左侧的view 原来宽30
    _leftView.frame = CGRectMake(0, 0, (KScreenWidth - scanViewW) * 0.5, self.frame.size.height);
    //右侧的view
    _rightView.frame = CGRectMake(self.frame.size.width-((KScreenWidth- scanViewW) * 0.5), 0, (KScreenWidth - scanViewW) * 0.5, self.frame.size.height);
    //最上部view
    _upView.frame = CGRectMake((KScreenWidth - scanViewW) * 0.5, 0, scanViewW,120);
    //底部view
    _downView.frame = CGRectMake((KScreenWidth - scanViewW) * 0.5, CGRectGetMaxY(_upView.frame) + scanViewW, scanViewW, KScreenHeight - (CGRectGetMaxY(_upView.frame) + scanViewW));
    //扫码框
    _centerView.frame = CGRectMake(CGRectGetMaxX(_leftView.frame), CGRectGetMaxY(_upView.frame), scanViewW, scanViewW);
    //扫描线
    _line.frame = CGRectMake((KScreenWidth - scanViewW) * 0.5, CGRectGetMaxY(_upView.frame), scanViewW, 2);
...
- (void)addAnimation{
    ...
    line.hidden = NO;
    CABasicAnimation *animation = [XYScanView moveYTime:2 fromY:[NSNumber numberWithFloat:0] toY:[NSNumber numberWithFloat:KScreenWidth-120 ] rep:OPEN_MAX];
